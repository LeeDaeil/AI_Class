import socket
from Exam_Model_Tester.TOOL import Check_CNS_Info
...
class CNS:
    ...
    def _make_mem_structure(self, max_len):
        ...
        idx = 0
        shared_mem = {}
        abs_path = Check_CNS_Info.get_abs_path()
        for file_name in [f'{abs_path}/ENVS/db.txt',
                          f'{abs_path}/ENVS/db_add.txt']:
    ...
    # logger
    def init_line(self):
        with open(f"{Check_CNS_Info.get_abs_path()}/{self.LoggerPath}/{self.file_name}.csv", 'w') as f:
            DIS = ''
    ...
    def save_line(self):
        with open(f"{Check_CNS_Info.get_abs_path()}/{self.LoggerPath}/{self.file_name}.csv", 'a') as f:
            DIS = ''
            for para_name in self.mem.keys():
